...
<body>
	<nav class="navbar navbar-default navbar-fixed-top navbar-inverse">
		<div class="container-fluid">
			...
			<div class="pull-right">
				<a href="#" data-toggle="modal" data-target="#modal_confirm" class="btn btn-danger navbar-btn">
	...
	<div class="col-md-8">
		<div class="alert alert-info" style="margin-top:100px;">File List from Storage</div>
		<div class="panel panel-default">
			<div class="panel-body alert-success">
				<table id="table" class="table table-bordered">
					...
					<tbody>
						<?php
							$query = mysqli_query($conn, "SELECT * FROM `student` WHERE `stud_id` = '$_SESSION[student]'") or die(mysqli_error($conn));
							$fetch = mysqli_fetch_array($query);
							$stud_no = $fetch['stud_no'];

							$query1 = mysqli_query($conn, "SELECT * FROM `storage` WHERE `stud_no` = '$stud_no'") or die(mysqli_error($conn));
		...
		<div class="alert alert-info" style="margin-top:20px;">File List from Storage2</div>
		<div class="panel panel-default">
			<div class="panel-body alert-success">
				<table id="table2" class="table table-bordered">
					<thead>
						<tr>
							...
							<th>File Type</th>
							<th>Date Uploaded</th>
							<th>Action</th> <!-- Add a new Action column -->
						</tr>
					</thead>
					<tbody>
						<?php
							$query2 = mysqli_query($conn, "SELECT * FROM `storage2`") or die(mysqli_error($conn));
							while($fetch2 = mysqli_fetch_array($query2)){
						?>
						<tr>
							<td><?php echo substr($fetch2['filename'], 0 ,30) . "..."; ?></td>
							<td><?php echo $fetch2['file_type']; ?></td>
							<td><?php echo $fetch2['date_uploaded']; ?></td>
							<td>
								<a href="download.php?store_id=<?php echo $fetch2['store_id']; ?>" class="btn btn-success"><span class="glyphicon glyphicon-download"></span> Download</a> <!-- Download button for storage2 -->
							</td>
						</tr>
						<?php
							}
